...
export async function getCustomerDetail(id: number) {
  ...
  const orders = await sql`
    SELECT o.id, o.invoice_code, o.total_amount, o.status, o.created_at, b.name as branch_name
    FROM orders o
...
export async function getCustomerBehaviors(id: number) {
  ...
  const vouchers = await sql`
    SELECT v.code, v.discount_type, v.discount_value, vr.created_at, vr.discount_applied
