...
export function TablesClient({ initialData, currentBranchId, isAllBranches = false, role = "STORE_ADMIN", branchName = "Cabang Utama", paymentMethods }: { initialData: any[], currentBranchId: number, isAllBranches?: boolean, role?: string, branchName?: string, paymentMethods?: any[] }) {
  ...
  const handleTableClick = async (t: any) => {
    ...
    if (t.status === "RESERVED") {
      setCustomerName(localStorage.getItem(`reserved_name_${t.id}`) || "")
      setTableNote(localStorage.getItem(`reserved_note_${t.id}`) || "")
    } else {
      setCustomerName("")
      setTableNote("")
    }
  ...
  const handleSetTableStatus = async (newStatus: string) => {
    ...
    if (newStatus === "RESERVED") {
      if (!customerName.trim() || !tableNote.trim()) {
        toast("Nama pelanggan dan catatan meja harus diisi untuk reservasi.", "error");
        return;
      }
    }
    ...
    try {
      ...
      if (newStatus === "RESERVED") {
        localStorage.setItem(`reserved_name_${actionTable.id}`, customerName);
        localStorage.setItem(`reserved_note_${actionTable.id}`, tableNote);
      } else if (newStatus === "AVAILABLE") {
        localStorage.removeItem(`reserved_name_${actionTable.id}`);
        localStorage.removeItem(`reserved_note_${actionTable.id}`);
      }
  ...
  const handleOpenSession = async () => {
    ...
    if (paxNum > actionTable.capacity) {
      toast(`Jumlah tamu tidak boleh lebih dari ${actionTable.capacity}`, "error")
      return
    }
  ...
  const handlePaySession = async () => {
    ...
    const totalWithTax = Math.round(Number(sessionDetail.grand_total || 0) * 1.11)
      ...
      <div className="print:hidden flex-1 overflow-y-auto font-sans text-foreground -m-3 sm:-m-6 p-4 sm:p-6 bg-background min-h-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-border">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              {viewMode === "grid" ? <><List size={14} /> <span>List View</span></> : <><Grid size={14} /> <span>Grid View</span></>}
            </button>
              ...
              <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                ...
                {panelMode === 'open' && (
                  <div className="space-y-6">
                    <div>
                      <label className="block text-xs font-bold text-foreground/90 mb-2 tracking-wider">Jumlah Tamu (Pax) - Maksimal {actionTable.capacity}</label>
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (val <= actionTable.capacity) setGuestCount(String(val));
                          }}
                          className="w-10 h-10 rounded-lg border border-border flex items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground transition-colors bg-background"
                        >
                          <Plus size={16} />
                    ...
                    <div>
                      <label className="block text-xs font-bold text-foreground/90 mb-2 tracking-wider">
                        Nama Pelanggan {actionTable.status === "AVAILABLE" ? <span className="text-destructive">*</span> : "(Opsional)"}
                      </label>
                      <input
                        type="text"
                        value={customerName}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-foreground/90 mb-2 tracking-wider">
                        Catatan Meja {actionTable.status === "AVAILABLE" ? <span className="text-destructive">*</span> : ""}
                      </label>
                      <textarea
                        rows={3}
                        value={tableNote}
                        ...
                        <div className="bg-background p-4 rounded-xl border border-border space-y-2">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-medium text-foreground/90">
                              {(() => {
                                const mins = Math.floor(sessionDetail.duration_minutes || 0);
                                if (mins >= 60) {
                                  const hours = Math.floor(mins / 60);
                                  const remainingMins = mins % 60;
                                  return `${hours} jam ${remainingMins > 0 ? `${remainingMins} menit` : ''} lalu`;
                                }
        ...
        <XenditQrisModal
          ...
          amount={Math.round(Number(sessionDetail?.grand_total || 0) * 1.11)}
          onSuccess={async () => {
            ...
            try {
              await fetch("/api/table-sessions", {
                ...
                body: JSON.stringify({
                  ...
                  action: "pay",
                  paymentMethod: paymentMethod,
                  cashAmount: Math.round(Number(sessionDetail?.grand_total || 0) * 1.11)
                })
              })
              toast("Pembayaran berhasil! Sesi meja telah ditutup.", "success")
            } catch (e: any) {
              toast("Gagal mengupdate status pesanan di server", "error")
            }
            ...
            const receiptData = {
              ...
              taxAmount: sessionDetail?.tax_amount || (Number(sessionDetail?.grand_total) * 0.11),
              discountAmount: sessionDetail?.discount_amount || 0,
              totalAmount: Math.round(Number(sessionDetail?.grand_total || 0) * 1.11),
              paymentMethod: paymentMethod,
              cashAmount: Math.round(Number(sessionDetail?.grand_total || 0) * 1.11),
              items: sessionDetail?.items || []
            }
